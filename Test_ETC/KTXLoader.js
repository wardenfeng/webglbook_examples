...
KTXLoader.prototype.load = function (url, callback)
{
	...
	request.onreadystatechange = (ev) =>
	{
		...
		{// 4 = "loaded"
			...
			{
				this.parse(request.response, true, callback);
			} else
			{
				console.err(url + " 加载失败！");
			}
		}
	}
	request.onprogress = (event) =>
	{
		console.log(`加载中 ${event.loaded} / ${event.total}`);
	}
	request.send();
}

KTXLoader.prototype.parse = function (buffer, loadMipmaps, callback)
{
